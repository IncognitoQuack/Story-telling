document.addEventListener('DOMContentLoaded', function() {
    const scrollDuration = 25000; // Duration in milliseconds (25 seconds)
    const scrollStep = 20; // Adjust this to control scroll speed

    let currentPosition = window.pageYOffset;
    const stepSize = 3; // Set a constant scroll step size for smoother scrolling

    function smoothScroll() {
        currentPosition += stepSize;
        window.scrollTo(0, currentPosition);

        // Continue scrolling for 25 seconds
        setTimeout(smoothScroll, scrollStep);
    }

    // Start the scrolling
    smoothScroll();

    // Stop scrolling after 25 seconds
    setTimeout(function() {
        clearTimeout(smoothScroll);
    }, scrollDuration);

    // Show the final message after 30 seconds from page load
    setTimeout(function() {
        // After 30 seconds, show the white background and the message
